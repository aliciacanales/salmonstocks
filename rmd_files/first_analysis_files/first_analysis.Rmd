...
p_a<-ggplot(data=var_out,aes(y=port_mean,x=var_res))+
  ...
  scale_x_continuous(breaks = c(1, 25), labels = c("Low", "High")) +
  scale_y_continuous(breaks = c(.25, 2.3), labels = c("Low", "High")) +
  #xlim(-0.5,1.5)+
  #ylim(0,3.5)+
  theme_classic() +
...
p2 <- p_a + annotate("segment",x=var_2019,y=mean_2019,xend=var_out$var_res[var_keep],yend=mean_list[var_keep],color="grey", linetype = 'dashed')
 

p2.1 <- p2 + geom_segment(aes(x = var_2019, 
                 y = mean_2019, 
                 xend = var_out$var_res[var_keep], 
                 yend = 1.15), 
             color = "grey", 
             linetype = "dashed") + 
             # arrow = arrow(type = 'closed', length = unit(0.3, "cm"))) +
  geom_point(aes(x=var_2019, y= mean_list[var_keep]), colour='red', size = 3)
p2.1
...
```{r}
p3 <- p2.1 + geom_segment(aes(x = var_2019, 
                 y = mean_2019, 
                 xend = var_out$var_res[var_keep], 
                 yend = mean_list[var_keep]), 
             color = "grey", 
             linetype = "dashed") 
             # arrow = arrow(type = "closed", length = unit(0.3, "cm")))
...
p4 <-p2.1+ annotate("segment",x=var_2019,y=mean_2019,xend=var_out$var_res[mean_keep],yend=mean_list[mean_keep],color="grey", linetype = 'dashed')
p5 <- p4 + geom_point(aes(x=var_2019, y=mean_2019), size=2.5) +
  geom_segment(aes(x = 2.9, 
                 y = mean_2019, 
                 xend = var_out$var_res[mean_keep] + .25 , 
                 yend = mean_list[mean_keep]), 
             color = "grey", 
             linetype = "dashed") + 
             # arrow = arrow(type = "closed", length = unit(0.3, "cm"))) +
  geom_point(aes(x= 1.0, y= mean_2019), colour='blue', size = 3)
p5 + theme(axis.text.x = element_text(size = 11),
           axis.text.y = element_text(size = 11),
           axis.title = element_text(size = 14)) +
  geom_segment(aes(x = var_2019 + 5,
                   y = mean_2019,
                   xend = var_2019 + .5,
                   yend = mean_2019),
               color = "black",
  ...
  geom_text(x = var_2019 + 8, y = mean_2019, label = "Baseline Portfolio", size = 4, 
            check_overlap = T)
...
ggsave("figures/mpt_spring23.png",bg="white")
```


```{r}
p4 <-p3+ annotate("segment",x=var_2019,y=mean_2019,xend=var_out$var_res[mean_keep],yend=mean_list[mean_keep],color="grey", linetype = 'dashed')

p5 <- p4 + geom_point(aes(x=var_2019, y=mean_2019), size=3) +
  geom_segment(aes(x = var_2019, 
                 y = mean_2019, 
                 xend = var_out$var_res[mean_keep], 
                 yend = mean_list[mean_keep]), 
             color = "grey", 
             linetype = "dashed", 
             arrow = arrow(type = "closed", length = unit(0.3, "cm")))
...
colnames(weights_mean_df) <- c("Alsea","Beaver","Coos","Coquille","Floras","Lower Umpqua","Middle Umpqua","Necanicum","Nehalem","Nestucca","North Umpqua","Salmon","Siletz","Siltcoos","Siuslaw","Sixes","South Umpqua","Tahkenitch","Tenmile","Tillamook","Yaquina")

test1 <- weights_mean_df %>% 
  pivot_longer(1:21,
               names_to = 'Population',
               values_to = 'Set of Weights')
test1
...
weights_var_df<- as.data.frame(var_out$weights[mean_keep]) %>% 
  round(3) 
colnames(weights_var_df) <- c("Alsea", "Beaver","Coos","Coquille","Floras","Lower Umpqua","Middle Umpqua","Necanicum","Nehalem","Nestucca","North Umpqua","Salmon","Siletz","Siltcoos","Siuslaw","Sixes","South Umpqua","Tahkenitch","Tenmile","Tillamook","Yaquina")
...
sum3 <- rbind(weights_mean_df, weights_var_df) %>%
  round(3) %>% 
  rename_with(str_to_title)

scenario_table <- merge(test, test1, by = "Population")
colnames(scenario_table) <- c('Population', 'Optimized Weights 1', 'Optimized Weights 2')
...
table_3_2019 <- sum3 %>% 
  ...
  kable_styling(bootstrap_options = "striped",
                position = "center", full_width = FALSE, html_font = 'Cambria', font_size = 15)
table_3_2019 %>% 
  ## weights for 2019 when variance is held
